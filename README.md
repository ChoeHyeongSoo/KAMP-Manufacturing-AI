...
- 브랜치: `main`(제출 기준) / 작업은 `<type>/<이니셜 3글자>-<작업내용>` 브랜치 → PR. type은 `feat/ fix/ docs/ test/ chore/ refactor/`. (예: `feat/CHS-01-injection-data-quality`, `docs/CHS-topic-selection-meeting`) PR 제목은 `[Data]`/`[Model]`/`[Analysis]`/`[Docs]`/`[Chore]` 구분 후 명사형 설명, merge commit으로 병합. 상세는 `CLAUDE.md` 참고.
