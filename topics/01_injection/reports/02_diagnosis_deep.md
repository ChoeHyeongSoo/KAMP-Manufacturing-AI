...
- 비인접 쌍(두 행의 인덱스 차이가 1이 아닌 쌍) 비율: **cn7 5.8%(35/605, gap 최대 5), rg3 0%(0/591)**. cn7 비인접
  쌍은 행 295~1192 구간에 몰려 있고, 서로 다른 두 쌍의 인덱스가 교차하는 패턴(예: 401·403 쌍과 402·404 쌍)이 보인다
  → 이 구간에서 기록 순서가 인터리브된 것으로 추정.
...
- **labeled에도 부분/완전 고정 행이 실제로 존재한다**: cn7은 마커 4개 이상 일치 42행(3.47%), 5개 이상 2행(0.17%),
  6개 전부 일치 0행. **rg3는 6개 전부 일치가 2행(0.17%) 있다** — 01의 "labeled에는 비가동 블록이 없다"는 단정은
  **부분적으로 틀렸다**(rg3에 극소수 존재).
- 비가동 행에서 나머지 18변수: **속도·압력·사이클·위치 계열(예: `Cycle_Time` std 0.093, `Cushion_Position` std
  0.001, `Max_Injection_Pressure` std 0.045)은 거의 완전히 고정**되는 반면, **온도 계열(`Barrel_Temperature_2~6`,
...
- labeled의 극소수 고정 행(rg3 2건)은 **완전 제거 대신 "비가동 근접도" 연속 플래그**(마커 일치 개수)로 남겨, 라벨
  있는 표본 손실을 피한다.
...
**발견**
- 블록 간 드리프트(5블록, PSI/KS 블록0 대비): **cn7·rg3 둘 다** 여러 변수에서 PSI가 5~10에 달한다(통상 PSI>0.25면
  "큰 변화"로 보는 기준을 훨씬 초과). cn7 최댓값은 `Injection_Time`(PSI 10.01), rg3 최댓값은 `Mold_Temperature_3`
  (PSI 8.46). 01에서 "cn7만 드리프트가 크다"고 서술한 것과 달리 **rg3도 강한 드리프트가 있으나, rg3는 불량이 전
...
- 파일 내부 순위(percentile rank)로 정규화한 24차원 벡터 기준, labeled→unlabeled 최근접거리 분포(cn7 mean 1.31·
  ...
  (거리<0.3)은 cn7·rg3 모두 0건(0/1,211, 0/1,182)**이다.
- 즉 값 기준으로 labeled 행이 unlabeled에 재등장하지 않는다는 01의 "원본 인덱스 안 겹침" 관찰이 근사 매칭으로도
  재확인됐다.

**시사점 → 활용 아이디어**
- unlabeled를 사전학습(오토인코더 등)·이상탐지·드리프트 참고용으로 자유롭게 사용해도 **labeled 평가셋 값 기준 누수
  위험은 낮다**(단, "순위 벡터 거리<0.3" 기준 안에서의 결론이므로 (추정) 표시).
- 실제로 unlabeled 사전학습을 도입한다면, 사전학습 전후 labeled CV 성능 차이를 별도로 재확인하는 것을 권장한다.
...
- 이산 변수(고유값<=20) 개수: **labeled cn7·rg3 둘 다 24개 중 15개**로 동일(`Injection_Time`, `Filling_Time`,
  ...
  추가로 뒷받침한다(추정).
- `Clamp_Open_Position`은 4개 파일 전부 상수(nunique=1).
...
**시사점 → 활용 아이디어**
- `Clamp_Open_Position`은 4파일 공통 제거 대상 1순위.
...
**발견**
- lag-1 자기상관 중앙값: **labeled cn7 0.859, unlabeled cn7 0.821, labeled rg3 0.801, unlabeled rg3 0.749** —
  **네 파일 전부에서 매우 높다**(변수 절반 이상이 0.7 이상). cn7 `Barrel_Temperature_6`은 lag-1 0.95, lag-5도 0.93로
  5샷 뒤까지 상관이 유지된다.
...
**시사점 → 활용 아이디어**
- 자기상관이 0.7~0.95로 매우 높으므로 **"직전 샷 값", "직전 샷 대비 차이"** 류의 파생 변수가 유효할 가능성이 높다
  (심사 5번 창의성 후보).
...
**시사점 → 활용 아이디어**
- cn7 규칙(F1 0.444)을 **cn7 전용 베이스라인 0호**로 채택하고, 심사 2번의 "베이스라인 포함 2개 이상 모델 비교"에서
...
| labeled/unlabeled 원본 인덱스 안 겹침 | 순위 기반 근사 매칭으로도 근접 매칭(거리<0.3) 0건 확인 — 값 기준 누수 없음(④) |
| cn7 드리프트 큼(rg3는 상대적으로 적다는 뉘앙스) | rg3도 다수 변수에서 강한 드리프트(PSI>5) — 다만 불량과 무관(③) |
...
| split 방식 | GroupKFold(X-키) + 시간 블록 split(예: 첫 1/5을 test) **병행 필수**, 랜덤 split 단독 사용 금지(자기상관 lag-1 0.75~0.86, 드리프트 PSI 5~10 모두 높음) |
| 결합 여부 | cn7·rg3 **별도 모델** 기본안(24변수 중 22개 KS p<0.001) |
| 제거 규칙 | `Clamp_Open_Position`(4파일 공통 상수) 제거. unlabeled idle 마스크(6마커 전부 고정) 유지 |
| 유지 규칙 | labeled 부분/완전 고정 행(rg3 2건 등)은 제거 대신 "비가동 근접도" 연속 플래그로 보존 |
| 파생 변수 후보 | 직전 샷 대비 변화량(lag-1 차분, 자기상관 0.7~0.95로 유효 가능성 높음), 마커 일치 개수/비가동 근접도, rg3 `Injection_Time`·`Filling_Time` 희귀 세그먼트 플래그, cn7 시간블록 상대위치(이동평균 대비 편차) |
| 베이스라인 | 규칙 기반(`outlier_type=='process'`) cn7 precision 0.60/recall 0.353/F1 0.444 — cn7 한정 0호 베이스라인으로 채택, rg3는 부적용(정밀도·재현율 0) |
