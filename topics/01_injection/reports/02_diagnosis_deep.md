...
- 충돌 쌍(같은 X, 다른 라벨) vs 일치 쌍(00/11)의 변수별 분포 차이(Mann-Whitney U, Bonferroni 보정):
  **cn7은 24개 중 10개 변수가 유의**(주로 `Mold_Temperature_3/4`, `Filling_Time`, `Plasticizing_Position`,
  ...
  0개**(최소 p_bonf=0.44) — rg3 충돌은 조건과 무관한 라벨 노이즈에 가깝다(**추정** — 24개 변수 각각의 단변량 검정
  기준이며, 변수 조합·비선형 조건까지 배제하는 것은 아니다).
...
**타깃 정의 후보 비교** (그룹 크기가 항상 1~2라 실제로 서로 다른 이진 결정을 내리는 것은 `max`와 `min` 둘뿐이다)

| 파일 | 정의 | 표본(행) | 양성(행) | 그룹(샷) 양성 | GroupKFold F1 (median / mean±std, n=5) |
|---|---|---|---|---|---|
| cn7 | max (충돌 쌍→양성) | 1,211 | 28 | 14 | 0.154 / 0.185±0.201 |
| cn7 | mean≥0.5 | 1,211 | 28 | 14(soft 8.5) | 0.154 / 0.185±0.201 (max와 이진 결정 동일) |
| cn7 | min (충돌 쌍→음성) | 1,211 | 6 | 3 | 1.000 / 0.600±0.548 (표본 과소로 신뢰 불가) |
| cn7 | exclude_conflict (충돌 쌍 제거) | 1,189 | 6 | 3 | 1.000 / 0.600±0.548 (min과 fold별로 완전히 동일) |
| rg3 | max (충돌 쌍→양성) | 1,182 | 50 | 25 | 0.078 / 0.061±0.059 |
| rg3 | mean≥0.5 | 1,182 | 50 | 25(soft 12.5) | 0.078 / 0.061±0.059 (max와 이진 결정 동일) |
| rg3 | min (충돌 쌍→음성) | 1,182 | **0** | 0 | **실행 불가(양성 0개)** |
| rg3 | exclude_conflict (충돌 쌍 제거) | 1,132 | **0** | 0 | **실행 불가(양성 0개)** |

- 그룹 크기가 항상 1~2이므로 `max`와 `mean≥0.5`는 **이진 결정이 수학적으로 동일**하다. 차이는 "기대 양성 수"에서만
  드러난다(cn7 14 vs 8.5, rg3 25 vs 12.5, 충돌을 0.5로 카운트).
- `min`과 `exclude_conflict`는 **양성 수가 같다**(cn7 6행/3그룹, rg3 0행/0그룹) — 차이는 표본 크기뿐(cn7 1,211행 vs
  1,189행, min이 22행 더 많다). cn7에서 GroupKFold F1도 fold별로 완전히 같은 값을 낸다.
- cn7의 `min`·`exclude_conflict` F1이 fold마다 0 또는 1로 튀는 이유: 5-fold 중 **2개(fold 0·1)는 test 양성이 0개라
  재현율이 정의되지 않아 zero_division=0으로 F1=0 처리**되고, 나머지 **3개(fold 2·3·4)는 test 양성이 2개씩**이며
  전부 F1=1.0(완벽 분류)이다(median 1.0, mean 0.60, std 0.55, n=5) — 표본 6개를 5-fold로 나눈 데서 오는 구조적
  불안정이지 모델이 실제로 뛰어나서가 아니다.
- rg3는 `min`·`exclude_conflict` 둘 다 **양성이 0개라 실행 자체가 불가능**하다(rg3 불량 25개 전부가 충돌 쌍이므로).

**시사점 → 활용 아이디어**
- 타깃 정의는 **`max` 확정**, `min`·`exclude_conflict`는 rg3에서 아예 못 쓰므로 주 정의 후보에서 제외하되, cn7의
  보수적 상한 참고용으로만 부기한다.
...
**unlabeled 완전 중복과 labeled 쌍 비교 (A-1, 01 리포트 점검)**

| 파일 | 중복 그룹 수 | 인접 비율(gap==1) | 최대 gap |
|---|---|---|---|
| labeled_cn7 | 605 | 94.2% | 5 |
| labeled_rg3 | 591 | 97.1% | 3 |
| unlabeled_cn7 | 6,468 | 99.98% | 98 |
| unlabeled_rg3 | 6,234 | 99.98% | 469 |

- unlabeled는 인접 비율이 거의 100%(사실상 전부 "바로 다음 행 반복")인 반면, labeled는 94.2%·97.1%로 소수(cn7
  5.8%·rg3 2.9%)가 몇 행 떨어져 등장한다(①에서 본 인터리브 구간과 동일). 두 현상 모두 "인접 반복이 압도적"이라는
  표면적 공통점은 있지만, labeled에만 인터리브된 비인접 사례가 섞여 있고 발생 빈도도 다르다(labeled 100% 전부
  중복 vs unlabeled 약 18%만 중복) → **완전히 같은 기록 메커니즘이라고 단정하기는 어렵다(추정, 미확정)**.
...
- **시기 교란 정량화(핵심 결과)**: cn7 `Mold_Temperature_3`의 전체 AUC(파일 전체)는 **0.894**인 반면, 불량 밀집
  구간(행 99~120)을 포함하는 블록(행 0~241) **내부**에서만 계산한 AUC는 **0.606**으로 급락한다 — **시기 교란으로
  인한 AUC 차이 약 0.29**(이 변수는 학습을 거치지 않은 단변량 순위 비교라 "누수"보다는 "그 구간 안에서는 변수
  순위와 불량 여부의 관계가 전체와 다르게 보인다"는 뜻으로 읽는다). `Mold_Temperature_4`도 0.892→0.657(차이 약
  0.23). rg3 최고 단일 AUC 변수 `Barrel_Temperature_5`는 전체 0.635 vs 시간블록 내부 0.511 — 애초에 절대 수준이
  낮아 시기 교란 여부와 무관하게 예측력이 거의 없다. 단 이 시간블록 내부 AUC는 **양성 4개(2쌍)만으로 계산**된
  값이라(`n_time_test_pos=4`) 표본이 극히 작고 신뢰하기 어렵다.
...
- `Mold_Temperature_3/4`처럼 전체-구간내부 AUC 격차가 큰 변수는 절대 수준이 아니라 **이동평균 대비 편차 등 상대적
  변화량으로 변환**해 시기 교란을 줄이는 파생 변수 설계를 검토한다.
...
- rg3 `Injection_Time`은 "단일 전환 시점"이 아니라 **주값(-0.029, top_share 98.6%)과 희귀값(-8.63/+8.57)이 총
  ...
  **불량 0건**(-8.63: 0/6행, +8.57: 0/10행, 합쳐서 0/16행).
- `Filling_Time`은 `Injection_Time`과 성격이 다르다: 희귀값 없이 **두 값이 108개 세그먼트(각 54개씩)로 거의
  균등하게 번갈아 나타나고, 불량률도 두 값이 비슷**하다(-1.193: 10건/488행=2.05%, 0.839: 15건/694행=2.16%, rg3
  전체 불량률 2.12%와 거의 같음). `Injection_Time`의 희귀값과 달리 `Filling_Time`의 두 값 자체는 불량 여부에
  대한 정보가 거의 없다.

**시사점 → 활용 아이디어**
- `Clamp_Open_Position`은 labeled 학습 입력에서 제거(labeled 상수라 정보 없음).
- rg3 `Injection_Time` 희귀값 세그먼트는 불량과 무관(오히려 0건)하므로 제거하지 말고, **"저해상도 셋포인트 전환
  샷 여부"** 이진 플래그로 남겨 모델이 활용 여부를 판단하게 한다. `Filling_Time`은 두 값의 불량률이 거의 같아
  (2.05%/2.16%) 이 변수 자체에서 별도 플래그를 만들 필요는 없다.
...
- rg3에서 규칙이 실패하는 이유는 ①·③에서 이미 살펴봤다: rg3 불량은 **피처와 무관한 라벨 충돌**이 대부분(25/25)으로
  보이므로(①의 Bonferroni 무유의 결과와 같은 근거 — **추정**, 24개 변수의 단변량 검정 기준이며 변수 조합까지
  배제하지는 않는다) 어떤 규칙을 세워도 원리적으로 못 잡을 가능성이 높다.
...
- rg3에는 이 규칙을 적용하지 않는다(정밀도·재현율 0) — rg3는 처음부터 "규칙/모델로 설명 안 되는 라벨 노이즈가
  지배적(추정)"이라는 사실을 그대로 반영해 무리한 성능 주장을 피한다.
...
| `Mold_Temperature` AUC 0.89는 "추정상" 시간 누수성 | 전체 AUC 0.894 vs 불량 밀집 구간 내부 AUC 0.606, 시기 교란으로 인한 차이 약 0.29로 정량 확인(③) |
...
| (미언급) unlabeled 완전 중복이 labeled 쌍과 같은 메커니즘인지 | 인접 비율 unlabeled 99.98% vs labeled 94.2%/97.1% — 표면적으로 비슷하나 labeled에만 인터리브된 비인접 사례가 섞여 있어 완전히 같다고 단정 어려움(미확정, 추정)(①) |

## 모델 단계 반영 사항

| 항목 | 결정 |
|---|---|
| 타깃 정의 | `max`(그룹 내 하나라도 불량이면 양성) 기본. `min`(충돌 쌍→음성)·`exclude_conflict`(충돌 쌍 제거)는 rg3에서 양성 0개로 사용 불가, cn7도 표본 과소(양성 6개, 5-fold 중 2개는 test 양성 0개)로 참고용에 그침 |
| 평가 라벨 한계 | 이 리포트의 F1·정밀도·재현율 수치는 전부 `max` 정의(충돌 쌍=양성) 기준이라 **양성이 부풀려진 라벨**로 계산됐다. `min` 정의로 다시 보면 rg3 양성은 0개가 된다 — 보고된 성능은 "라벨 정의에 따라 크게 달라지는 값 중 하나"로 읽어야 하며, 모델 단계에서는 max/min 두 정의로 성능 상·하한을 함께 보고한다 |
...
| 파생 변수 후보 | 직전 쌍 대비 변화량(쌍 단위 lag-1 차분, 쌍 복제 제거 후 lag-1 중앙값 0.61~0.75), rg3 `Injection_Time` 희귀 세그먼트 플래그(`Filling_Time`은 두 값 불량률이 거의 같아 제외), cn7 시간블록 상대위치(이동평균 대비 편차) |
...
- `src/drift.py` — 블록 드리프트, 전체/시간블록 내부 AUC, 자기상관, cn7·rg3 분포 비교
